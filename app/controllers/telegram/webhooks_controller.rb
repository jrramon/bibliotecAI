module Telegram
  ...
  # - any text or photo from a linked chat is queued as :pending; the
  #   host claude-worker drains it via TelegramMessageJob and replies
  #   (Slice 4). We send a "typing…" chat action up front so the user
  #   gets feedback while claude generates the answer.
  # - photos from a linked chat are downloaded inline, attached to the
  #   TelegramMessage, and routed by the LLM via the
  #   process_book_cover_photo / process_shelf_photo MCP tools
  #   (Slice 10). The webhook itself no longer decides between cover
  #   and shelf; Claude does, from the caption + context.
  class WebhooksController < ApplicationController
    ...
    def create
      ...
      if message[:photo].present?
        handle_photo(user: user, chat_id: chat_id, update_id: update_id, message: message)
        return head :ok
      end
    ...
    # photo) and attach it to a fresh TelegramMessage; the caption (if
    # any) becomes the text. The LLM downstream decides via the MCP
    # photo tools whether this is a single cover or a whole shelf —
    # no routing logic lives here anymore.
    #
    # Download failures (Telegram CDN/API hiccups) abort without
    # persisting: we tell the user and let them retry. Active Storage
    # blob lifecycle isn't worth half-attached rows.
    def handle_photo(user:, chat_id:, update_id:, message:)
      ...
      tg_message = TelegramMessage.new(
        user: user,
        chat_id: chat_id,
        update_id: update_id,
